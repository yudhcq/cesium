    ...
    float split)
{
    ...
    outAlpha = czm_branchFreeTernary(outAlpha == 0.0, -czm_epsilon7, outAlpha);
    ...
    //   if (alphaZero) { outColor = vec3(0.0); }
    //
    // but using czm_branchFreeTernary will cause portions of the tile that are
    // alpha-zero in the additional pass to render as black instead of blending
    // with the previous pass:
    //   outColor = czm_branchFreeTernary(alphaZero, vec3(0.0), outColor);
    //
    // So instead, sanitize against divide-by-zero and correct on return.
